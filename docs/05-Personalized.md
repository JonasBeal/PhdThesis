...
In addition to these examples proposed in the original article, an application to cell line data is proposed in section \@ref(PROFILE-CL) to link to the next chapters. \@ref(appendix-cl) In all cases, samples and cell lines will sometimes be referred to as patients for the sake of simplicity.

Details on the different datasets can be found in the appendix, section \@ref(appendix-datasets).

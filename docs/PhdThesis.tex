...
The first modeling approach, that we will call mechanistic, consists in
building the box by imitating what we think is the process of data
generation (Figure \ref{fig:boxes}B). This integration of a priori
...
deterministic model defined a priori. All in all, in a purely
...
considered to measure them.

The second approach, often called statistical modeling or machine
learning, does not necessarily seek to reproduce the natural process of
data generation but to find the function allowing the best prediction of
\(Y\) from \(X\) (Figure \ref{fig:boxes}C). Pushed to the limit, they
are ``idealized version of the data we gain from immediate observation''
\citep{frigg2020models}, thus providing a phenomenological description.
The methods and algorithms used are then intended to be sufficiently
flexible and to make the fewest possible assumptions about the
relationships between variables or the distribution of data. Without
...
\subsection{A tale of prey and
predators}\label{a-tale-of-prey-and-predators}
...
This is already the case in Figure \ref{fig:tcga} since genetic
alterations have been grouped in several categories called pathways. A
pathway is group of biological entities, and associated chemical
reactions, working together to control a specific cell function like
apoptosis or cell division. The meaning of these groupings derives quite
simply from the description of hallmarks. Indeed, if the aim of these
cells is to inactivate each of these protective functions, then it is
relevant to think not by gene but by function. Inactivating only one of
the genes associated with the function may therefore be sufficient and
it is no longer necessary to inactivate the others. Numerous alterations
in a large number of genes result often in few key impaired pathways,
like alterations of cell cycle or angiogenesis for
instance\citep{jones2008core}. While the data inform us about the
underlying biology, their analysis benefits in return from the a priori
knowledge of the relationships between the different proteins and how
they contribute to the various functions. Increasingly, the biological
variables are not interpreted separately but in relation to each other
\citep{barabasi2004network}. This is reflected in the emergence of more
and more resources to summarize and represent signaling pathways and
associated networks such as SIGNOR \citep{perfetto2016signor}, OmniPath
\citep{turei2016omnipath} or the Atlas of Cancer Signaling Network
\citep{kuperstein2015atlas}. Like other diseases, cancer then goes from
a genetic disease to a network disease \citep{del2010diseases} and one
can study how all kinds of genetic alterations affect the wiring of
these networks and modify the cellular functions leading to the
previously described cancer hallmarks as depicted schmatically in Figure
\ref{fig:circuit}. In short, the richness of the data did not make it
less necessary to use a priori knowledge in order to make the analyses
more interpretable and more robust .
...
\ref{fig:pathways} and is the most recent view of the tools and data
available to the modeller who wants to dissect more deeply the
mechanisms at work.
...
\chapter{Computational modeling of cancer: from complex disease to
systems
biology}\label{computational-modeling-of-cancer-from-complex-disease-to-systems-biology}
...
\caption[Cancer is an old disease]{\textbf{Dissecting disease using non-computational
models.} Rembrandt, \emph{The Anatomy Lesson of Dr Nicolaes Tulp}, 1634,
oil on canvas, Mauritshuis museum, The Hague}\label{fig:anatomy}
\end{figure}
...
Different kinds of models: scales --\textgreater{} focus to signaling.
We will model a cell

Many reviews Timeline of mathematics and cancer
\citep{byrne2010dissecting}. Mainly focused on angiogenesis,
carconigenesis and tumour growth. Different formalisms depending on
scales review \citep{bellomo2008foundations}. Focus on cellular scale
and bigger.

Integrative and scales. Many mathematical models are cell-based
\citep{anderson2008integrative}

PKPD and regimens \citep{barbolosi2016computational}

Tumour growth \citep{benzekry2014classical}

review \citep{altrock2015mathematics}

\section{Features of complex systems: emergence, non-linearity,
feedback}\label{features-of-complex-systems-emergence-non-linearity-feedback}
...
\citep{helikar2008emergent}

\section{Need for integrated
approaches}\label{need-for-integrated-approaches}

network and complex elements. Examples of loops
\citep{kitano2002computational} \citep{kitano2002systems} prior
knowledge \citep{ferranti2017value}

\section{Various formalisms and
objectives}\label{various-formalisms-and-objectives}

\citep{le2015quantitative} tyson ode examples breast
\citep{tyson2011dynamic} and general \citep{tyson2003sniffers} Boris
Kholodenko

With data also \citep{bouhaddou2018mechanistic}

...
on the literature and the description of causal inference in the
preprint \citet{beal2020causal}.
\EndKnitrBlock{summarybox}
...
choice of systemic modelling approaches and in particular mechanistic
...
mechanistic or not. How then can we assess the relevance of these models
in their ability to guide patient treatment?
...
currently under revision and is available as a pre-print
...
\subsection{Starting from pre-clinical data: patient-derived
xenografts}\label{starting-from-pre-clinical-data-patient-derived-xenografts}
...
\caption[Analysis on observed data without confounder]{\textbf{Analysis on observed data without
confounder.} (A) Directed acyclic graphs}\label{fig:PDX-dense}
\end{figure}
...
their associated treatments, as proposed in the SHIVA clinical trial by
\citet{le2015molecularly}.}\label{fig:SHIVA}
\end{figure}
...
Mechanistic models of cancer pathways are particularly well suited to
study response to treatment. Their explicit representation of genes and
proteins makes it possible to simulate the effect of different
therapeutic interventions. On the basis of this observation, it is
possible to imagine the following situation
...
sensitivities to several different drugs for each tumour. The PDX
dataset used in this thesis is the one published by \citet{gao2015high}.
The original dataset contains 281 different tumours of origin (sometimes
called PDX models, in the sense of a biological model) and 63 tested
drugs, not all drugs having been tested for all tumours and some drugs
have been tested with tissue-specific patterns (Figure
\ref{fig:PDX-appendix}). 192 of these tumours have also been

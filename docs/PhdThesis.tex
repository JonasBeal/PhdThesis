...
approaches as central to managing this profusion. Thus, Figure
\ref{fig:pubmed} shows that while the number of scientific articles
devoted to cancer has increased drastically since the 1950s (panel A),
the proportion of these same articles mentioning \emph{models},
\emph{networks} or \emph{computational} approaches has also increased
(panel B), illustrating a change in paradigms.
...
Once the structure of the model has been defined, it is possible to
refine certain parameters according to the chosen mathematical
formalism. Finally, the model is often confronted with new data to check
its consistency with the biological behaviour studied or possibly make
new predictions. The use of biological data can occur at each of these
stages and it is important to understand and distinguish between the
possible methods and the expected contributions. The first steps prove
to be very largely common to the different modelling formalisms and thus
constitute a very general framework for reflection that we will however
illustrate as much as possible by logical modelling approaches.
...
simpler version of this cycle is described in {[}le2015quantitative{]}.}\label{fig:cycle}
\end{figure}
...
bistable and switches \citep{markevich2004signaling}

tyson ode examples breast \citep{tyson2011dynamic} and general
\citep{tyson2003sniffers}

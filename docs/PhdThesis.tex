...
of prior knowledge can take different forms. In this thesis it will
...
perfectly deterministic model defined \emph{a priori}. All in all, in a
purely mechanistic approach, the nature of the relations between
entities should be linked to biological processes and the parameters in
the model all have biological definitions in such a way that it could
even be considered to measure them directly. For example, the
coefficient \(2\) multiplying \(X_2X_3\) can correspond to a
stoichiometric coefficient or a reaction constant which have a
theoretical justification or are accessible by experimentation. In some
fields of literature these models are sometimes called mathematical
models because they propose a mathematical translation of a phenomenon,
which does not start from the data in a bottom-up approach but rather
from a top-down theoretical framework. In this thesis we will adhere to
the \emph{mechanistic model} name, which is more transparent and less
ambiguous compared to other approaches also based on mathematics,
without necessarily the other characteristics described above.
...
the kind of tools and data available to the modeler who wants to dissect
more deeply the mechanisms involved in cancer.
...
comparison with tumor experimental growth \citep{benzekry2014classical}.
...
question to validated personalized logical models.}}\label{fig:BRAF-GA}
\end{figure}
...
result, the resulting personalized mechanical models have proven to be
...
In the case of mechanistic molecular signalling models, this

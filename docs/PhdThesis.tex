...
L121-3 du code de l'éducation} la langue de rédaction de cette thèse
devrait être le français. Sa rédaction en anglais doit en conséquence
s'accompagner d'un résumé détaillé en français qui est fourni ci-dessous
en suivant une progression similaire au manuscrit qui peut alors être
titré:
...
\addcontentsline{toc}{section}{Modélisation et cancer}

\subsection*{La modélisation scientifique, la complexité et
l'abstraction}\label{la-moduxe9lisation-scientifique-la-complexituxe9-et-labstraction}
\addcontentsline{toc}{subsection}{La modélisation scientifique, la
complexité et l'abstraction}
...
modèles mécanistiques, est leur nécéssaire simplicité. Parce leur
existence découlen de la complexité des phénomènes étudiés, les modèles
...
\subsection*{De la dérégulation de la machinerie cellulaire au
cancer}\label{de-la-duxe9ruxe9gulation-de-la-machinerie-cellulaire-au-cancer}
\addcontentsline{toc}{subsection}{De la dérégulation de la machinerie
cellulaire au cancer}
...
manifestations cliniques de la maladie, résumées par une proliferation
incontrôlée de cellules, sont connues depuis des siècles. La
compréhension des mécanismes biologiques sous-jacents ne s'est
approfondie qu'à partir de la découverte de l'ADN comme support de
l'information génétique, au milieu du XXème siècle. Le cancer est
aujourd'hui reconnu comme une \textbf{maladie génétique} dont l'origine
réside essentiellement dans des altérations de l'ADN. La
\textbf{combinaison de certaines altérations}, souvent accumulées au fil
du temps, est alors nécéssaire pour inactiver les différentes
protections biologiques qui prémunissent en temps normal contre une
...
compréhension: le cancer n'est plus seulement une maladie génétique mais
également une maladie de réseaux (Figure \ref{fig:pathways}).

Sur le plan technologique, la recherche profite également depuis le
début du XXIème siècle de données beaucoup plus abondantes, issues
notamment du séquençage à haut débit, qui permettent une vision plus
globale en renseignant sur des milliers de gènes et \textbf{différents
types de données omiques}: génomique (ADN), transcriptomique (ARN),
protéomique (protéines) etc. Ces données sont notamment disponibles
publiquement pour des \textbf{milliers de patients} atteints de cancer
(consortimu TCGA par exemple) mais aussi pour de \textbf{nombreux
modèles précliniques} comme des lignées cellulaires issues de patients.

\subsection*{Modélisation mécanistique du cancer : d'une maladie
complexe à la biologie des
systèmes}\label{moduxe9lisation-muxe9canistique-du-cancer-dune-maladie-complexe-uxe0-la-biologie-des-systuxe8mes}
\addcontentsline{toc}{subsection}{Modélisation mécanistique du cancer :
d'une maladie complexe à la biologie des systèmes}
...
analyser et les modéliser. Ce thèse se focalise sur la
...
\textbf{formalisme logique}, plus parsimonieux mais qualitatif et qui
sera décrit en détail plus avant.

L'un comme l'autre ont pour vocation de répliquer le phénomène tumoral
étudié (activation d'une voie de siganlisation, impact d'une mutations
...
phénomène stumoraux, ces modèles mécanistiques moléculaires du cancer
...
\addcontentsline{toc}{section}{Des modèles logiques personnalisés de
cancer}

\subsection*{Principes de modélisation logique et intégration des
données}\label{principes-de-moduxe9lisation-logique-et-intuxe9gration-des-donnuxe9es}
\addcontentsline{toc}{subsection}{Principes de modélisation logique et
...
interpétée comme une abstraction de son activité: \(0\) si inactive
(gène non transcrit, protéine en trop faible concentration etc.), \(1\)
si active (gène transcrit, protéine phosphorylée, ARN stabilisé etc.).
Ces entités sont reliées entre elles par des règles logiques composées à
...
à partir de la littéraure comme source primaire d'information.
...
\subsection*{Personnalisation des modèles logiques : méthode et
validation
pronostique}\label{personnalisation-des-moduxe8les-logiques-muxe9thode-et-validation-pronostique}
\addcontentsline{toc}{subsection}{Personnalisation des modèles logiques
: méthode et validation pronostique}

Les modèles définis à partir de la litérature sont par construction
...
de réponse au traitement. À partir d'un réseau générique, une des
possibilités est d'utiliser les données biologiques des différentes
tumeurs pour spécifier, personnaliser les modèles. Les méthodes
existantes proposent pour la plupart d'\textbf{entraîner et d'optimiser
les paramètres des modèles} à l'aide d'une fonction d'objectif
pré-définie. Cela requiert cependant des données riches, souvent des
données de perturbations qui sont rarement accessibles en dehors de
certains modèles précliniques comme les lignées cellulaires.

Une autre approche a été suivie dans cette thèse consistant à
personnaliser les modèles en \textbf{interpétant biologiquement des
...
sa nature qualitative et parsimmonieuse étant adaptée aux données
utilisées. Peut-on alors obtenir de ces données des modèles
mécanistiques de cancer personnalisés et interpétables cliniquement ? La
\textbf{méthode PROFILE} conçue durant cette thèse cherche à répondre à
cette question \textbf{sans entraînement des modèles}.
...
\ref{fig:logical-personalization}). La première infère l'\textbf{effect
...
biologique sera traduite en forçant la variable correspondant du modèle
...
\subsection*{Des modèles logiques personnalisés pour interpréter la
réponse aux
traitements}\label{des-moduxe8les-logiques-personnalisuxe9s-pour-interpruxe9ter-la-ruxe9ponse-aux-traitements}
\addcontentsline{toc}{subsection}{Des modèles logiques personnalisés
pour interpréter la réponse aux traitements}
...
cible et son mode d'action sont suffisament connus. Cette analyse n'est
...
précis de cette analsye. Une étude plus ciblée a donc été menée par la
...
personalisé à l'aide des données de mutations et d'ARN issues de lignées
cellaires des deux cancers concernés. La réponse des modèles
personalisés à l'inhibition de BRAF est ensuite évaluée en observant le
score de \emph{Proliferation} atteints par ces modèles avec et sans
inhibiteurs. La réduction de \emph{Proliferation} engendrée par
l'inhibition simulée de BRAF est comparée aux résulats expérimentaux
issus de l'inhibition de BRAF sur les mêmes lignées cellulaires par un
traitement ou par CRISPR/Cas9. Les corrélations significatives entre
sensibilités expérimentales et simulées valident la pertinence des
modèles personnalisés, en particulier ceux résultant de l'intégration
conjointe des données de mutation et d'ARN (Figure
\ref{fig:BRAF-results}). Les modèles personalisés apparaissent par
ailleurs comme un outil d'investigation qualitative porteur de sens en
permettant de mettre en lumière et en contexte certains mécanismes de
résistance, liés à CRAF par exemple (Figure \ref{fig:BRAF-interactive}).
Une comparaison avec des méthodes d'apprentissage automatique, ici des
forêts aléatoires, est effectuée afin de souligner la complémentarité
des approches et l'intérêt des modèles logiques personnalisés sans
...
\addcontentsline{toc}{section}{Quantification statistique de l'impact
clinique des modèles}

\subsection*{Flux d'information dans les modèles mécanistiques de
cancer}\label{flux-dinformation-dans-les-moduxe8les-muxe9canistiques-de-cancer}
\addcontentsline{toc}{subsection}{Flux d'information dans les modèles
mécanistiques de cancer}

\subsection*{Essais cliniques et inférence
causale}\label{essais-cliniques-et-infuxe9rence-causale}
\addcontentsline{toc}{subsection}{Essais cliniques et inférence causale}

\subsection*{Inférence causale pour la médecine de
précision}\label{infuxe9rence-causale-pour-la-muxe9decine-de-pruxe9cision}
\addcontentsline{toc}{subsection}{Inférence causale pour la médecine de
précision}

...
invisible(lapply(X =  c("knitr", "tidyverse", "magrittr",  "ggplot2", "ggpubr", "patchwork", "cowplot", "kableExtra", "UpSetR", "eulerr", "survival", "survminer", "paletteer"),
       FUN = require,
       character.only = TRUE))
...
META_clin <- readRDS("data/appendix/META_clin.rds") %>%
  mutate(OS_STATUS=if_else(OS_STATUS=="LIVING", 0, 1))  %>%
  mutate(OS_STATUS=if_else(OS_MONTHS<time_limit & OS_STATUS==1,1,0),
         OS_MONTHS=if_else(OS_MONTHS>time_limit,time_limit,OS_MONTHS)) %>%
  filter(PAM50!="NC")
KM <- survfit(Surv(OS_MONTHS,OS_STATUS)~PAM50,
                  data=META_clin)
...
```{r Fumia, echo=FALSE, out.width = "80%", fig.cap='(ref:Fumia-caption)', fig.scap='Graphical abstract of PROFILE method to personalize logical models with omics data', fig.align='center'}

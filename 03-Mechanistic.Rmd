...
knitr::asis_output('\\epigraph{"How remarkable is life? The answer is: very. Those of us who deal in networks of chemical reactions know of nothing like it... How could a chemical sludge become a rose, even with billions of years to try."}{George Whitesides (The improbability of life, 2012)}')
```
...
pm_cancer <- read_csv("data/mechanistic/PubMed_Cancer.csv", skip = 1) %>%
  filter(Year<2020) %>% rename(Cancer=Count)
pm_model <- read_csv("data/mechanistic/PubMed_Model.csv", skip = 1) %>%
  filter(Year<2020) %>% rename(Model=Count)
pm_networks <- read_csv("data/mechanistic/PubMed_CancerNetwork.csv", skip = 1) %>%
  filter(Year<2020) %>% rename(Network=Count)
pm_compmath <- read_csv("data/mechanistic/PubMed_CancerCompMath.csv", skip = 1) %>%
...
```{r cycle, echo=FALSE, out.width = "70%", fig.cap='(ref:cycle-caption)', fig.scap='Modeling a biological network: an iterative and cyclical process', fig.align='center'}
knitr::include_graphics("fig/cycle.png")
```
...
```{r toyraf, echo=FALSE, out.width = "90%", fig.cap='(ref:toyraf-caption)', fig.scap='Schematic example of logical and ODE modeling around MAPK signaling', fig.align='center'}
knitr::include_graphics("fig/toyraf.png")
```
...
```{r odelogic, echo=FALSE, out.width = "90%", fig.align='center', eval=TRUE}

text_tbl <- data.frame(
  `Quantitative modeling` = c("Ordinary differential equation (ODE) models",
                             "Direct translation of biological quantities, usually continuous",
                             "Quantitatively accurate and temporal simulation of an experimental phenomenon",
                             "Direct confrontation with experimental data; precise; linear representation of time",
                             "Difficulty determining or fitting parameters"),
  `Qualitative modeling` = c("Logical models",
                             "Abstract representation of activity levels, usually discrete",
                             "Coarse-grained simulation of qualitative phenotypes",
                             "Faster design; easy translation of literature-based assertions; simulation of perturbations",
                             "More difficult to link to data; lower precision"),
  check.names = FALSE, stringsAsFactors = FALSE)

rownames(text_tbl) <- c("Example formalism",
                        "Type of variables",
                        "Objective",
                        "Advantages",
                        "Drawbacks")
...
if (is_html_output()){
  kable(text_tbl, caption = '(ref:odelogic-caption)') %>%
    kable_styling(bootstrap_options = "striped", full_width = F) %>%
    row_spec(0, bold = T, color = "white", background = "#808080", align = 'center') %>%
    column_spec(1, bold = T, border_right = T, color = "white", background = "#808080")
  ...
  kable(text_tbl, caption = '(ref:odelogic-caption)') %>%
    ...
    column_spec(1, bold = T, border_right = T,  width = "6em"#, color = "white", background = "#808080"#,
                ) %>%
...
```{r fey, echo=FALSE, out.width = "90%", fig.cap='(ref:fey-caption)', fig.scap='Mechanistic modeling of JNK pathway and survival of neuroblastoma patients, as described by Fey et al.', fig.align='center'}
...
```{r netdrug, echo=FALSE, out.width = "90%", fig.cap='(ref:netdrug-caption)', fig.scap='Network model of oncogenic signal transduction in ER+ breast cancer, including some drugs and their targets', fig.align='center'}
knitr::include_graphics("fig/netdrug.png")
```

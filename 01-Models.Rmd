...
invisible(lapply(X =  c("knitr", "tidyverse", "magrittr",  "ggplot2", "ggpubr", "patchwork", "cowplot", "kableExtra", "ggwordcloud", "tm", "magick", "primer", "latex2exp", "ggpmisc", "textreadr"),
       FUN = require,
       character.only = TRUE))
...
```{r visual-thesaurus, echo=FALSE, out.width = "90%", fig.cap ='(ref:visual-thesaurus-caption)', fig.scap="Network visualization of \\emph{model} thesaurus entries", fig.align='center'}
...
```{r planets, echo=FALSE, out.width = "90%", fig.cap ='(ref:planets-caption)', fig.scap="Orrery, planets and models", fig.align='center', fig.height=4, fig.width=8}
...
```{r pubmed-tree, echo=FALSE, out.width = "90%", fig.cap ='(ref:pubmed-tree-caption)', fig.scap="Tree visualization of \\emph{model} semantic context in cancer-related literature", fig.align='center'}
...
```{r boxes, echo=FALSE, out.width = "60%", fig.cap ='(ref:boxes-caption)', fig.scap="Different modeling strategies.", fig.align='center'}
knitr::include_graphics("fig/boxes.png")
```
...
```{r mechstat, echo=FALSE, out.width = "90%",fig.align='center'}

text_tbl <- data.frame(
  `Mechanistic modeling` = c("Seeks to establish a mechanistic relationship between inputs and outputs",
                             "Presupposes and investigates causal links between the variables",
                             "Capable of handling small datasets",
                             "Once validated, can be used as a predictive tool in new situations possibly difficult to access through experimentation ",
                             "Difficult to accurately incorporate information from multiple space and time scales due to constrained specifications",
                             "Evaluated on closeness to data and ability to make sense of it"),
  `Statistical modeling` = c("Seeks to establish statistical relationships between inputs and outputs",
                             "Looks for patterns and establishes correlations between variables",
                             "Requires large datasets",
                             "Can only make predictions that relate to patterns within the data supplied",
                             "Can tackle problems with multiple space and time scales thanks to flexible specifications",
                             "Evaluated based on predictive performance"),
check.names = FALSE, stringsAsFactors = FALSE)

if (is_html_output()){
  kable(text_tbl, caption = '(ref:mechstat-caption)') %>%
    kable_styling(bootstrap_options = "striped", full_width = F) %>%
    column_spec(1, border_right = T) %>%
    column_spec(2) %>%
    row_spec(0, bold = T, color = "white", background = "#808080", align = 'c') %>%
    pack_rows("Definition", 1, 1) %>%
    pack_rows("Pros and cons", 2, 6)
...
```{r lotka, echo=FALSE, out.width = "90%", fig.cap ='(ref:lotka-caption)', fig.scap="Some analyses around Lotka-Volterra model of a prey-predator system", fig.align='center', fig.show='hold', eval=TRUE, fig.height=8, fig.width=8}
animal_colors <- c(Lynx="#AF967DFF", Hare="#7890A8FF")

lv <- readxl::read_xlsx("data/models/hare_lynx_data.xlsx", skip = 1) %>%
  select(Year, Lynx, Hare) %>%
  filter(Year>=1900, Year<=1930)
lv[,"dLynx"] <- (lv[c(2:nrow(lv),NA),"Lynx"]-lv[c(NA, 1:(nrow(lv)-1)),"Lynx"])/2
lv[,"dHare"] <- (lv[c(2:nrow(lv),NA),"Hare"]-lv[c(NA, 1:(nrow(lv)-1)),"Hare"])/2

dL <- ggplot(lv, aes(x=Hare, y=(dLynx/Lynx))) +
  geom_point(color="grey50") +
  geom_smooth(method = 'lm', formula = y~x,
              color="black", linetype="dashed") +
  stat_poly_eq(formula = y~x,
               aes(label =..eq.label..),
               parse=T, label.y.npc = 0.10) +
  theme_pubclean() +
  theme(axis.title.y = element_text(size = 10)) +
  labs(y=TeX("$\\frac{1}{Lynx} \\times \\frac{d.Lynx}{dt}$"))

dH <- ggplot(lv, aes(x=Lynx, y=(dHare/Hare))) +
  geom_point(color="grey50") +
  geom_smooth(method = 'lm', formula = y~x,
              color="black", linetype="dashed") +
  stat_poly_eq(formula = y~x,
  ...
  theme_pubclean() +
  theme(axis.title.y = element_text(size = 10)) +
  labs(y=TeX("$\\frac{1}{Hare} \\times \\frac{d.Hare}{dt}$"))
  
# ggplot(lv, aes(x=Lynx, y=Hare)) +
...
predpreyLV <- function (t, y, params) {
    ...
    with(as.list(params), {
        dH.dt <- a1 * H - a2 * L * H
        dL.dt <- -b1 * L + b2 * L * H 
        return(list(c(dH.dt, dL.dt)))
    })
}
 
lv_params <- c(a1=0.82, a2=0.0298, b1=0.509, b2=0.0129) 
lv_time <- seq(1900, 1930, by=.1) # Set time here
lv_res <- ode(c(H0=12.82,L0=7.13), lv_time, predpreyLV, lv_params) %>%
  as.data.frame %>%
  rename(Hare=H0, Lynx=L0) %>%
  pivot_longer(cols=c("Hare", "Lynx"), names_to = "Animal", values_to = "Population")

comb <- bind_rows(pivot_longer(lv, cols=c("Lynx", "Hare"),
                               names_to = "Animal", values_to = "Population") %>%
                    mutate(Type="Original"),
                  rename(lv_res, Year=time) %>% mutate(Type="Model"))

# ggplot(comb, aes(x=Year, y=Population, color=Animal)) +
#   geom_line() +
#   geom_point(data = filter(comb, Year%%1==0)) +
#   facet_grid(Type~.) +
#   scale_color_manual(values=animal_colors) +
#   theme_pubclean() +
#   theme(legend.key = element_rect(fill = NA, colour = NA, size = 0.25))

plot_data <- ggplot(filter(comb, Type=="Original"), aes(x=Year, y=Population, color=Animal)) +
  geom_line() +
  geom_point(data = filter(comb, Type=="Original")) +
  scale_color_manual(values=animal_colors) +
  theme_pubclean() +
  theme(legend.key = element_rect(fill = NA, colour = NA, size = 0.25),
  ...
  labs(color="Animal:",
       y="Population from data\n(thousands of fur pelts)")

plot_model <- ggplot(filter(comb, Type=="Model"), aes(x=Year, y=Population, color=Animal)) +
  geom_line(show.legend = FALSE) +
  geom_point(data = filter(comb, Year%%1==0, Type=="Model"), show.legend = FALSE) +
  scale_color_manual(values=animal_colors) +
  theme_pubclean() +
  theme(legend.key = element_rect(fill = NA, colour = NA, size = 0.25),
        legend.title = element_text(face="bold"),
        axis.title.y = element_text(size = 10)) +
  labs(color="Animal:",
       y="Population from model\n(thousands of fur pelts)")

plot_data / (dL|dH) / plot_model +

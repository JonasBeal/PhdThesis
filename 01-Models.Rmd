...
invisible(lapply(X =  c("tidyverse", "magrittr", "textreadr", "ggplot2", "ggwordcloud", "ggpubr", "tm", "cowplot", "magick", "kableExtra", "knitr"),
       FUN = require,
       character.only = TRUE))
...
if (is_html_output()){
  ...
  kable(text_tbl, caption = '(ref:mechstat-caption)') %>%
    ...
    row_spec(0, bold = T, color = "white", background = "#808080")%>%
    pack_rows("Definition", 1, 1) %>%
    pack_rows("Pros and cons", 2, 6)
}
...
```{r lotka, echo=FALSE, out.width = "90%", fig.cap ='(ref:lotka-caption)', fig.scap="Some analyses around Lotka-Volterra model of a prey-predator system", fig.align='center', fig.show='hold', eval=FALSE}
...
ggplot(lv, aes(x=Hare, y=(dLynx/Lynx))) +
  geom_point() + geom_smooth(method = 'lm') +
  stat_poly_eq(formula = y~x,
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")),
               parse=T)

ggplot(lv, aes(x=Lynx, y=(dHare/Hare))) +
  geom_point() + geom_smooth(method = 'lm') +
  stat_poly_eq(formula = y~x,
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")),
               parse=T)
  
dat <- pivot_longer(lv, cols=c("Lynx", "Hare"), names_to = "Animal", values_to = "Population") %>%
...
params1 <- c(a1=0.4, a2=0.02, b1=0.8, b2=0.02) 
Time <- seq(1845, 1935, by=.1) # Set time here
LV.out <- ode(c(H0=20,L0=30), Time, predpreyLV, params1) %>% as.data.frame %>%
  rename(Hare=H0, Lynx=L0) %>%
  pivot_longer(cols=c("Hare", "Lynx"), names_to = "Animal", values_to = "Population")

res <- ggplot(LV.out, aes(x=time, y=Population)) +
  geom_line(aes(color=Animal)) +
  geom_point(aes(color=Animal)) +
  theme_pubclean()
...
comb <- bind_rows(pivot_longer(lv, cols=c("Lynx", "Hare"),
                  ...
                  rename(LV.out, Year=time) %>% mutate(Type="Model"))

ggplot(comb, aes(x=Year, y=Population, color=Animal)) +
  geom_line() +
  facet_grid(.~Type)

ggplot(comb, aes(x=Year, y=Population, color=Type)) +
  geom_line() +
  facet_grid(.~Animal)
```
